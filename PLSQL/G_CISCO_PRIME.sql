CREATE OR REPLACE PACKAGE G_CISCO_PRIME AS 
  ...
  *
  */
  PROCEDURE P_CALC_SUM_CGN_STATS_RWK(P_FECHA IN VARCHAR2);
  --******************************************************--
 ...
 */
  ...
  *
  */
  PROCEDURE P_CALC_SUM_CPU_MEM_DEV_AVG(P_FECHA IN VARCHAR2);
  /**
  *
  */
  PROCEDURE P_CALC_SUM_CPU_MEM_DEV_AVG_RWK(P_FECHA IN VARCHAR2);
  --******************************************************--
  --                      INTERFACE                       --
  --******************************************************--
  /**
  * Funcion para retornar los datos de los equipos existentes en CSCO_INTERFACE_HOUR para un
  * dia en particular, se calcula para el dia anterior (SYSDATE - 1)
  */
  FUNCTION GET_CSCO_EQUIPOS(P_EXCLUIR_EQUIPO IN VARCHAR2 DEFAULT 'NINGUNO') RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED;
  /**
  * Funcion para retornar las interfaces asociadas a un equipo
  */
  FUNCTION GET_CSCO_INTERFACE_POR_EQUIPO(P_EQUIPO IN VARCHAR2) RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED;
  /**
  *
  */
...
CREATE OR REPLACE PACKAGE BODY G_CISCO_PRIME AS
  ...
  --
  BEGIN
    -- Limpio la tabla para los valore IBWH en caso de que existan (Rework)
    DELETE FROM CSCO_CGN_STATS_IBHW WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
    --
    OPEN cur(p_fecha_desde,p_fecha_hasta);
    LOOP
  ...
  BEGIN
    -- calculo el dia anterior
    SELECT  TO_CHAR(TO_DATE(P_FECHA,'DDMMYYYY')-1,'DD.MM.YYYY')
    INTO  V_FECHA
    FROM  DUAL;
    --
    ...
    IF (TRIM(V_DIA) = 'SUNDAY') OR (TRIM(V_DIA) = 'DOMINGO') THEN
      ...
      --
      P_CGN_STATS_IBHW_INS(V_DOMINGO,V_SABADO);
      --
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CGN_STATS_IBHW_INS',0,'NO ERROR','FIN DE SUMARIZACION IBHW');
    END IF;
  END P_CALC_SUM_CGN_STATS;
  --**--**--**--
  PROCEDURE P_CALC_SUM_CGN_STATS_RWK(P_FECHA IN VARCHAR2) IS
    V_DOMINGO VARCHAR2(10 CHAR) := '';
    V_SABADO  VARCHAR2(10 CHAR) := '';
    V_FECHA   VARCHAR2(10 CHAR) := '';
  BEGIN
    -- P_FECHA viene con fmt, YYYY-MM-DD, lo paso a DD.MM.YYYY
    SELECT  TO_CHAR(TO_DATE(P_FECHA,'YYYY-MM-DD'),'DD.MM.YYYY')
    INTO V_FECHA
    FROM DUAL;
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CGN_STATS_DAY_INS',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY P_CGN_STATS_DAY_INS('||V_FECHA||')');
    ...
    --
    IF F_RECALCULAR_IBHW(V_FECHA) = 1 THEN
      V_DOMINGO := TO_CHAR(TO_DATE(V_FECHA,'DD.MM.YYYY')+ (1-TO_CHAR(TO_DATE(V_FECHA,'DD.MM.YYYY'),'D')),'DD.MM.YYYY'); 
      V_SABADO  := TO_CHAR(TO_DATE(V_FECHA,'DD.MM.YYYY')+ (7-TO_CHAR(TO_DATE(V_FECHA,'DD.MM.YYYY'),'D')),'DD.MM.YYYY');
      --
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CGN_STATS_IBHW_INS',0,'NO ERROR','COMIENZO DE SUMARIZACION IBHW P_FECHA_DOMINGO => '||
      ...
      P_CGN_STATS_IBHW_INS(V_DOMINGO,V_SABADO);
      --
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CALC_SUM_CGN_STATS_RWK',0,'NO ERROR','FIN DE SUMARIZACION IBHW');
    END IF;
  END P_CALC_SUM_CGN_STATS_RWK;
  --******************************************************--
  --                CSCO_CPU_MEM_DEVICE_AVG               --
  --******************************************************--
  ...
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    -- Limpiar los valores antes de insertar
    DELETE FROM CSCO_CPU_MEM_DEVICE_AVG_DAY WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
    --
    OPEN cur(P_FECHA_DESDE,P_FECHA_HASTA);
    LOOP
  ...
  PROCEDURE P_CSCO_CPU_MEM_DEVICE_AVG_IBHW(P_FECHA_DESDE IN VARCHAR2,P_FECHA_HASTA IN VARCHAR2) AS
    ...
    BEGIN
      EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
      -- Limpio los valores antes de insertar
      DELETE FROM CSCO_CPU_MEM_DEVICE_AVG_IBHW WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
      --
      OPEN cur(p_fecha_desde,p_fecha_hasta);
      LOOP
  ...
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    -- Limpio los valores antes de insertar
    DELETE FROM CSCO_CPU_MEM_DEVICE_AVG_BH WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
    --
    OPEN cur(P_FECHA_DESDE,P_FECHA_HASTA);
    LOOP
  ...
  --**--**--**--
  PROCEDURE P_CALC_SUM_CPU_MEM_DEV_AVG(P_FECHA IN VARCHAR2) AS
    V_DIA     VARCHAR2(10 CHAR) := '';
    V_FECHA   VARCHAR2(10 CHAR) := '';
    V_SABADO  VARCHAR2(10 CHAR) := '';
    V_DOMINGO VARCHAR2(10 CHAR) := '';
  BEGIN
    ...
    IF (TRIM(V_DIA) = 'SUNDAY') OR (TRIM(V_DIA) = 'DOMINGO') THEN
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_IBHW',0,'NO ERROR','COMIENZO DE SUMARIZACION IBHW P_FECHA_DOMINGO => '||
                                  ...
                                  ' P_FECHA_SABADO => '||V_SABADO);
      --
      P_CSCO_CPU_MEM_DEVICE_AVG_IBHW(V_DOMINGO,V_SABADO);
      --
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_IBHW',0,'NO ERROR','FIN DE SUMARIZACION IBHW');
    END IF;
  END P_CALC_SUM_CPU_MEM_DEV_AVG;
  --**--**--**--**--
  PROCEDURE P_CALC_SUM_CPU_MEM_DEV_AVG_RWK(P_FECHA IN VARCHAR2) AS
    V_DOMINGO VARCHAR2(10 CHAR) := '';
    V_SABADO  VARCHAR2(10 CHAR) := '';
    V_FECHA   VARCHAR2(10 CHAR) := '';
  BEGIN
    ...
    FROM DUAL;
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_DAY',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY P_CGN_STATS_DAY_INS('||V_FECHA||')');
    P_CSCO_CPU_MEM_DEVICE_AVG_DAY(V_FECHA,V_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_DAY',0,'NO ERROR','FIN DE SUMARIZACION DAY');
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_BH',0,'NO ERROR','COMIENZO CALCULO BH P_CGN_STATS_BH_INS('||V_FECHA||')');
    P_CSCO_CPU_MEM_DEVICE_AVG_BH(V_FECHA,V_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_BH',0,'NO ERROR','FIN CALCULO BH');
    --
    -- Verificar si hay que re-calcular IBHW
    ...
    IF F_RECALCULAR_IBHW(V_FECHA) = 1 THEN
      ...
      G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_CPU_MEM_DEVICE_AVG_IBHW',0,'NO ERROR','FIN DE SUMARIZACION IBHW');
    END IF;
  END P_CALC_SUM_CPU_MEM_DEV_AVG_RWK;
  --******************************************************--
  --                     INTERFACE                        --
  --******************************************************--
  FUNCTION GET_CSCO_EQUIPOS(P_EXCLUIR_EQUIPO IN VARCHAR2 DEFAULT 'NINGUNO') RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    FOR DATO IN (SELECT  NODE
                FROM     CSCO_INTERFACE_HOUR
                WHERE    TO_CHAR(FECHA,'DD.MM.YYYY') = TO_CHAR(SYSDATE-1,'DD.MM.YYYY')
                AND      NODE NOT IN (P_EXCLUIR_EQUIPO)
                GROUP BY NODE
                ORDER BY 1 ASC) LOOP
      PIPE ROW(CSCO_NODO_INTERFAZ_OBJ(DATO.NODE));
    END LOOP;
  END GET_CSCO_EQUIPOS;
  --**--**--**--
  FUNCTION GET_CSCO_INTERFACE_POR_EQUIPO(P_EQUIPO IN VARCHAR2) RETURN CSCO_NODO_INTERFAZ_TAB PIPELINED AS
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    FOR DATO IN (SELECT  INTERFAZ
                FROM CSCO_INTERFACE_HOUR
                WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = TO_CHAR(SYSDATE-1,'DD.MM.YYYY')
                AND NODE = P_EQUIPO
                GROUP BY INTERFAZ
    ...
    END LOOP;
  END GET_CSCO_INTERFACE_POR_EQUIPO;
  --**--**--**--
  ...
  BEGIN
    ...
    FROM    DUAL;
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_INTERFACE_DAY',0,'NO ERROR','COMIENZO DE SUMARIZACION DAY P_CSCO_INTERFACE_DAY('||V_FECHA||')');
    P_CSCO_INTERFACE_DAY(V_FECHA,V_FECHA);
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_INTERFACE_DAY',0,'NO ERROR','FIN DE SUMARIZACION DAY');
    --
    G_ERROR_LOG_NEW.P_LOG_ERROR('P_CSCO_INTERFACE_BH',0,'NO ERROR','COMIENZO CALCULO BH P_CSCO_INTERFACE_BH('||V_FECHA||')');
    P_CSCO_INTERFACE_BH(V_FECHA,V_FECHA);
  ...
  PROCEDURE P_CALC_SUM_INTERFACE_RWK(P_FECHA IN VARCHAR2) IS
    ...
    V_FECHA   VARCHAR2(10 CHAR) := '';
  BEGIN
    -- P_FECHA viene con fmt. YYYY-MM-DD, lo cambio a DD.MM.YYYY
    SELECT  TO_CHAR(TO_DATE(P_FECHA,'YYYY-MM-DD'),'DD.MM.YYYY')
    INTO  V_FECHA
    FROM DUAL;
    --
