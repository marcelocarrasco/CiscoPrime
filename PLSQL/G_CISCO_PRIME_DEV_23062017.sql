...
CREATE OR REPLACE PACKAGE BODY G_CISCO_PRIME AS
  ...
  BEGIN
    ...
    -- Limpio la tabla para la fecha dada
    DELETE FROM CSCO_CGN_STATS_DAY WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
    --
  ...
  BEGIN
    EXECUTE IMMEDIATE 'alter session set nls_date_format = ''DD.MM.YYYY HH24:MI:SS''';
    -- Limpio la tabla para la fecha dada
    DELETE FROM CSCO_CGN_STATS_BH WHERE TO_CHAR(FECHA,'DD.MM.YYYY') = P_FECHA_DESDE;
    --
    OPEN cur(p_fecha_desde,p_fecha_hasta);
    LOOP
